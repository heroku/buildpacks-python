## Rough buildpack steps
lookup known Python version files
fallback if version not set (with message)
resolve version range to exact version
reject invalid Python versions
Python EOL version warning
install Python runtime + configure env vars
install Pip/setuptools/wheel
install user deps + handle pip cache
compileall? (https://github.com/GoogleCloudPlatform/buildpacks/blob/663180f2cf1fbfdee4a889f6c9d17ee2eb9091bf/pkg/python/python.go#L163-L170)

## Python runtime manifest
Should we handle SHA256 now, or wait until later?
If later, should we use simple version ranges/lists instead of a full mapping?
How to store EOL? By major version or duplicate for each version?
How to handle asset URLs? Generate on the fly or duplicate full URL per runtime?
Nest minor versions inside major, or flat list?
...
- Invalidating site-packages increases runtime from: 0.25s -> 1.4s (small project), 0.6s -> 9s (large project), 0.6s -> 34s (large project using hashes)
- Invalidating pip cache too increases runtime from: 1.4s -> 5.7s (small project), 9s -> 38s (large project), 34s -> 38s (large project using hashes)
...
### .python-version
X.Y.Z
doesn't support X.Y unless using a plugin

## Demo
- Version handling
  - No one way to specify Python version (runtime.txt is a Heroku-specific construct)
  - Current buildpack version handling buggy
  - Common mistakes (omitting prefix, em dash etc)
  - Result<Option> or Option<Result>
- Package resolution
  - Only one version of a package possible, so conflicts are an issue
  - CNB spec says later buildpacks should not modify layers of earlier buildpacks
  - Issues around `PYTHONPATH` and shadowing, meaning problematic to install components into many arbitrary directories
  - As such more limited number of buckets to install things into
  - Also bugs with pip and `--target` / `--prefix`
  - Google now on their fourth approach for how they install deps after encountering issues with the others
  - Timestamp based .pycs
- Package manager
  - What package managers exist for Python
  - Limitations of pip:
    - doesn't sync (eg removals)
    - deps don't have to be pinned (plus transitive)
    - requirements files can include others
    - `--upgrade` makes unnecessary requests to PyPI even for pinned deps
    - with `--upgrade` new versions could drop a dependency and it doesn't get cleaned up either
- Tips for CNB development
